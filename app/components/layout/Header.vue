  ...
  <header class="header">
    ...
    >
      <SocialIcon
        v-if="options.github"
        platform="github"
        :username="options.github"
      />
      <SocialIcon
        v-if="options.linkedin"
        platform="linkedin"
        :username="options.linkedin"
      />
    </div>
    <NuxtLinkLocale
      to="/"
    ...
    <ClientOnly>
      ...
      >
        ...
        <button
          class="mode-switcher"
          :aria-label="$t('header.cycleModes')"
          @click="cycleModes"
        >
          <Transition
            name="fade"
          ...
          >
            <UnoIcon
              ...
              key="dark"
              class="i-line-md-lightbulb-off-twotone"
            />
            <UnoIcon
              v-else-if="mode.preference === 'light'"
              key="light"
              class="i-line-md-lightbulb-twotone"
            />
            <UnoIcon
              v-else
              key="system"
              class="i-line-md-light-dark"
            />
          </Transition>
        </button>
        <ul>
        ...
        <Teleport to="#modals">
          <div
            v-if="contactVisible"
            class="fixed w-100vw h-100lvh inset-0 p-10 backdrop-blur-sm z-110"
            @click.self="contactVisible = false"
          >
            <UnoIcon
              class="i-line-md-close absolute text-xl left-50% -translate-x-1/2 top-5 block cursor-pointer text-foreground"
              @click="contactVisible = false"
            />
            <ContactForm
              :is-visible="contactVisible"
              class="text-background"
              @update:is-visible="contactVisible = $event"
            />
          </div>
        </Teleport>
      </nav>
...
const titleEmoji = useState<string | undefined>('header-title-emoji', () => {
  if (!props.withEmoji) {
    return undefined
  }

  const emojiList = (props.options.isBirthday ? props.emojis?.birthday : props.emojis?.normal) ?? []
  return emojiList.length ? random(emojiList) : undefined
})
...
const computedName = computed(() => {
  ...
  if (!props.withEmoji || !titleEmoji.value) {
    return title
  }
  ...
  return `${first} ${titleEmoji.value} ${rest.join(' ')}`
})
